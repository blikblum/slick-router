...
import { routerLinks as routerLinksMiddleware } from '../../lib/middlewares/router-links'
import { defineCE, expect } from '@open-wc/testing'
import { LitElement, html } from 'lit'
import sinon from 'sinon'
import 'jquery'

import { routerLinks } from '../../lib/lit/routerLinks.js'
...
class ParentView extends LitElement {
  ...
  render() {
    return html`
      <div
        id="parent-view-root"
        ${routerLinks({ params: this.getParamsCallback, query: getQueryCallback })}
      >
      ...
      <div ${routerLinks({ params: { id: 10 }, query: { test: 'xxx' } })}>
...
class GrandChildView extends LitElement {
  ...
  render() {
    return html`
      <div ${routerLinks({ query: { other: 'xx' } })}>
...
describe('routerLinks directive', () => {
  ...
  beforeEach(() => {
    ...
    router.use(wc)
    router.use(routerLinksMiddleware)
    router.listen()
  })
  ...
  describe('when elements are added dynamically', () => {
    it('should generate href attributes in anchor tags with route attribute', function (done) {
      router.transitionTo('parent').then(async function () {
        ...
        `).appendTo(parentEl.renderRoot.querySelector('div#parent-view-root'))
