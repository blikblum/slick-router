...
async function resolveComponents (transition, routes) {
  const result = []
  ...
  for (const route of routes) {
    ...
    if (Component) {
      if (typeof Component === 'function' && !(Component.prototype instanceof HTMLElement)) {
        Component = routeComponentMap[route.name] || (routeComponentMap[route.name] = resolveModule(await Component()))
      }
...
async function next (transition) {
  ...
  const activated = await resolveComponents(transition, transition.routes.slice(changingIndex))
